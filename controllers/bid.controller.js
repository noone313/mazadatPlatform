...
export const createBid = async (req, res) => {
  try {
    ...
    if (auction.status !== "active") return res.status(400).json({ message: "المزاد غير نشط" });

    const highestBid = auction.Bids.length > 0 ? parseFloat(auction.Bids[0].amount) : parseFloat(auction.start_price);
    if (parseFloat(amount) <= highestBid) {
      return res.status(400).json({ message: `المزايدة يجب أن تكون أعلى من المبلغ الحالي ${highestBid}` });
    }

    // إنشاء المزايدة
    const newBid = await Bid.create({ auction_id, bidder_id, amount });
...
if (auction.Bids.length > 0) {
  ...
  if (previousHighestBidderId !== bidder_id) {
    ...
    // بث الإشعار عبر سوكيت خاص بالمستخدم
    ...
    // إرسال عبر Socket.io مع السعر الحالي
